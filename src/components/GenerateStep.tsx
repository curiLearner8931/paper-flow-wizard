...
const GenerateStep: React.FC<StepProps> = ({ 
  ...
  const generateExamPaper = async () => {
    ...
    try {
      // Progress updates
      const progressSteps = [
        { step: 20, message: "Preparing exam data..." },
        { step: 40, message: "Uploading template..." },
        { step: 60, message: "Processing with backend..." },
        { step: 80, message: "Generating documents..." },
        { step: 100, message: "Finalizing papers..." }
      ];
      ...
      // Add exam data as JSON
      formData.append('examData', JSON.stringify(examData));

      console.log('Sending to backend:', examData);
      console.log('Template file:', templateFile?.name);

      // Call backend API
      const response = await fetch('https://exampapercreator.onrender.com/generate-exam', {
        method: 'POST',
        body: formData,
      });
      ...
      // Set download links from backend response
      setDownloadLinks({
        docx: result.docxUrl || '#',
        pdf: result.pdfUrl || '#'
      });
      ...
      toast({
        ...
        description: error instanceof Error ? error.message : "There was an error generating your exam papers. Please try again.",
        variant: "destructive",
      });
    } finally {
          ...
          <CardContent className="space-y-4">
            ...
            <div className="flex items-center space-x-2 p-4 bg-blue-50 rounded-lg border border-blue-200">
              ...
              <p className="text-sm text-blue-800">
                <strong>Generated by:</strong> Backend service at exampapercreator.onrender.com
              </p>
            </div>
          </CardContent>
        </Card>
